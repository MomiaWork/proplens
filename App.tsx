...
import { syncDataIfNeeded } from './src/device/dataSync';
import { getPropertyQueryService, enrichTransactions } from './src/device/queryEngine';
import type { PropertyCard, SchoolDistrictFieldResult } from './src/device/queryEngine';
...
export default function App() {
  ...
  async function runSync() {
    ...
    try {
      const result = await syncDataIfNeeded(({ message, ratio }) => setSync({ status: 'syncing', message, ratio }));
      getPropertyQueryService(result.updated);
      const enrichment = await enrichTransactions((done, total) => {
        if (total > 0) {
          setSync({
            status: 'syncing',
            message: `分析交易資料中 (${done}/${total})，首次同步較久...`,
            ratio: done / total,
          });
        }
      });
      setSync({
        status: 'ready',
        notice: enrichment.stoppedByRateLimit
          ? `系統地理編碼額度已用盡，還有 ${enrichment.remaining} 筆交易待分析，下次開啟時會自動接續。目前查詢結果的同分區樣本數可能偏低。`
          : undefined,
      });
    } catch (err) {
      setSync({
    ...
    <SafeAreaView style={styles.safeArea}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          {sync.status === 'ready' && sync.notice && <Text style={styles.warning}>{sync.notice}</Text>}
