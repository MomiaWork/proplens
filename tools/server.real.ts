import { buildRealPropertyQueryService } from "./dev/realWorld";
import { startPropertyServer, PORT } from "./dev/httpServer";

/**
 * Same /property?address=... contract as tools/server.ts, but backed by the
 ...
 * tools/dev/realWorld.ts) plus data/taichung-zoning.geojson to exist (see
 * tools/dev/convertZoningShpToGeoJson.ts). The app needs no changes
 * to point at this instead of tools/server.ts — same host/port/contract.
 */
async function main() {
