 ...
 */
export async function syncDataIfNeeded(onProgress?: (progress: SyncProgress) => void): Promise<SyncResult> {
  ...
  for (const [index, file] of DATA_FILES.entries()) {
    ...
    const label = `下載資料 (${index + 1}/${DATA_FILES.length})`;
    onProgress?.({ message: `${label} ${file.name}`, ratio: 0 });
